...
# Look for geotop in PATH,
# if it's not there try to look in env variable GEOTOP_EXE.
geotop_exe = which("geotop")
if not geotop_exe:
    geotop_exe_env = os.getenv("GEOTOP_EXE")
    if geotop_exe_env and os.path.isfile(geotop_exe_env) \
            and os.access(geotop_exe_env, os.X_OK):
